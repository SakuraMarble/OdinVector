...
namespace pipeann {
  template<typename T, typename TagT>
  void SSDIndex<T, TagT>::do_filter_beam_search(const T *query1, uint32_t k_search, uint32_t mem_L, uint32_t l_search,
    ...
    std::priority_queue<Neighbor> exact_result_heap;
    std::priority_queue<Neighbor> pq_candidate_heap;
    tsl::robin_set<uint32_t> exact_computed_set(4096);
      ...
      for (auto &frontier_nhood : frontier_nhoods) {
        ...
        if (dyn_search_l) {
          ...
          for (cur = 0; cur < cur_list_size; ++cur) {
            ...
            if (exclude_nodes->find(tag_dyn) == exclude_nodes->end() && candidate_set->find(tag_dyn) != candidate_set->end()) {
              ++tot;
              if (tot == original_l_search)
                break;
            }
  ...
  size_t SSDIndex<T, TagT>::filter_beam_search(const T *query, const _u64 k_search, const _u32 mem_L, const _u64 l_search,
    ...
    this->do_filter_beam_search(query, (_u32) k_search, mem_L, (_u32) l_search, (_u32) beam_width, expanded_nodes_info,
                                &candidate_set, nullptr, stats, deleted_nodes, dyn_search_l);
