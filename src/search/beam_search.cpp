...
namespace pipeann {
  template<typename T, typename TagT>
  void SSDIndex<T, TagT>::do_filter_beam_search(const T *query1, uint32_t mem_L, uint32_t l_search, const uint32_t beam_width,
    ...
    while (k < cur_list_size) {
      ...
      for (auto &frontier_nhood : frontier_nhoods) {
        ...
        if (dyn_search_l) {
          ...
          for (cur = 0; cur < cur_list_size; ++cur) {
            uint32_t tag = id2tag(retset[cur].id);
            if (exclude_nodes->find(tag) == exclude_nodes->end() && candidate_set->find(tag) != candidate_set->end()) {
              ++tot;
              if (tot == original_l_search) {
                break;
              }
  ...
  size_t SSDIndex<T, TagT>::filter_beam_search(const T *query, const _u64 k_search, const _u32 mem_L, const _u64 l_search,
    ...
    std::shared_lock lk(merge_lock);
    std::vector<Neighbor> expanded_nodes_info;
    tsl::robin_set<uint32_t> candidate_set(candidate_ids.begin(), candidate_ids.end()); // 使用tsl::robin_set加速筛选

    this->do_filter_beam_search(query, mem_L, (_u32) l_search, (_u32) beam_width, expanded_nodes_info, &candidate_set, nullptr, stats,
                         deleted_nodes, dyn_search_l);
    _u64 res_count = 0;
    for (uint32_t i = 0; i < l_search && res_count < k_search && i < expanded_nodes_info.size(); i++) {
      res_tags[res_count] = id2tag(expanded_nodes_info[i].id);
