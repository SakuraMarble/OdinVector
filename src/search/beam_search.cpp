...
namespace pipeann {
  ...
  void SSDIndex<T, TagT>::do_filter_beam_search(const T *query1, uint32_t k_search, uint32_t mem_L, uint32_t l_search, const uint32_t beam_width,
    ...
    std::priority_queue<Neighbor> exact_result_heap; // 用于存放计算过精确距离且符合条件的节点
    std::priority_queue<Neighbor> pq_candidate_heap; // 用于存放内层循环用PQ距离海选的潜力节点
