...
void InvertedIndexSearcher::load(const std::string& index_path) {
    ...
    // 5. 初始化/清空 Mem 层和墓碑
    _mem_index.clear();
    _mem_universal_bitmap = roaring::Roaring();
    _deleted_bitmap = roaring::Roaring();
...
void InvertedIndexSearcher::write_bitmap(std::ofstream& out, const roaring::Roaring& rb) {
    size_t size = rb.getSizeInBytes(); 
    uint32_t size32 = static_cast<uint32_t>(size);
    out.write(reinterpret_cast<const char*>(&size32), sizeof(size32));
    std::vector<char> buffer(size);
...
roaring::Roaring InvertedIndexSearcher::query_bitmap(const std::vector<int>& tags) {
    ...
    for (int tag : tags) {
        ...
        roaring::Roaring current_tag_bitmap;
...
void InvertedIndexSearcher::merge(const std::string& new_index_path) {
    ...
    // 1. 合并通用 Bitmap
    roaring::Roaring new_universal = (_disk_universal_bitmap | _mem_universal_bitmap) - _deleted_bitmap;
    ...
    // 3. 合并具体的 Tag Bitmap
    std::map<int, roaring::Roaring> new_index;
