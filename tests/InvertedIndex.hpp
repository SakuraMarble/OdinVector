...
private:
    std::map<int, Roaring> _inverted_index;
    Roaring _universal_bitmap;
    void write_bitmap(std::ofstream& out, const Roaring& rb);
};
...
public:
    ...
    Roaring query_bitmap(const std::vector<int>& tags);

private:
    std::map<int, Roaring> _disk_index;
    Roaring _disk_universal_bitmap;

    std::map<int, Roaring> _mem_index;
    Roaring _mem_universal_bitmap;
    Roaring _deleted_bitmap;
    ...
    Roaring read_bitmap(std::ifstream& in);
    void write_bitmap(std::ofstream& out, const Roaring& rb);
