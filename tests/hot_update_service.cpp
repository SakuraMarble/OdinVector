...
class HotUpdateService {
 public:
  HotUpdateService(const std::string &index_prefix, const std::string &data_bin, unsigned L_disk, unsigned nthreads,
...
template<typename T, typename TagT>
std::string execute_command(HotUpdateService<T, TagT> &svc, const std::string &line) {
  ...
  if (cmd == "HELP") {
    return "OK commands: HELP STATUS STATS INSERT_RANDOM n [threads] INSERT_ROWS csv_rows [threads] "
           "DELETE_BASE_RANDOM n DELETE_INSERTED n SEARCH_ROW row k [L] [beam] "
           "SEARCH_TAG tag k [L] [beam] SEARCH_RANDOM nq k [L] [beam] MERGE [threads] QUIT SHUTDOWN";
  }
...
template<typename T, typename TagT>
void client_loop(int cfd, HotUpdateService<T, TagT> &svc) {
  (void) send_all(cfd, "OK hot_update_service ready. type HELP\n");
  while (true) {
...
void run_server(const std::string &index_prefix, const std::string &data_bin, unsigned L_disk, unsigned nthreads,
  ...
  HotUpdateService<T, TagT> svc(index_prefix, data_bin, L_disk, nthreads, beamwidth, nodes_to_cache);
  ...
  LOG(INFO) << "hot_update_service listening on 0.0.0.0:" << port;
