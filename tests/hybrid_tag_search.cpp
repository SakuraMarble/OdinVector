...
public:
    ...
    Roaring parse(const std::string& expression) {
...
private:
    ...
    Roaring evaluate_rpn(const std::string& rpn) {
        std::stack<Roaring> stk;
        ...
        while (ss >> token) {
            if (std::isdigit(token[0])) {
                ...
                Roaring b = stk.top(); stk.pop();
                Roaring a = stk.top(); stk.pop();
                if (token == "&") a &= b;
                else if (token == "|") a |= b;
                else if (token == "-") a -= b;
                stk.push(a);
        ...
        return stk.empty() ? Roaring() : stk.top();
    }
};
...
#pragma omp parallel num_threads(num_threads)
        {
            ...
            for (int64_t i = 0; i < (int64_t)query_num; i++) {
                ...
                try {
                    Roaring candidates = parser.parse(query_labels[i]);
